...
public interface JdbcExecutor {
     ...
     *
     * @param statementId iBatis 语句 ID
     * @param pageable    Spring Data 的分页信息（包含页码、页大小和排序）
     * @param params      查询参数（可以是 Map、POJO 或基本类型）
     * @return 分页结果（包含总数和当前页行）
     */
    Page<Map<String, Object>> pageQuery(String statementId, Pageable pageable, Object params);

    /**
     * 分页查询（不含 params 参数的便捷重载）。
     *
     * @param statementId iBatis 语句 ID
     * @param pageable    Spring Data 的分页信息
     * @return 分页结果（包含总数和当前页行）
     */
    default Page<Map<String, Object>> pageQuery(String statementId, Pageable pageable) {
