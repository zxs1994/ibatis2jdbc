...
public class TestParameterUtils {
		...
		if (includeOptional) {
			for (String property : meta.dynamicProperties) {
				if (!parameters.containsKey(property)) {
					parameters.put(property,
							sampleValue(property, '#', meta.iterateProperties.contains(property), variantIndex));
				}
			}
		}
		return parameters;
	...
	private static Object sampleValue(String property, char marker, boolean iterate, int variantIndex) {
		...
		if (iterate) {
			return variantIndex == 0 ? Arrays.asList("EVENT_A", "EVENT_B")
					: Arrays.asList("EVENT_X", "EVENT_Y", "EVENT_Z");
		}
		if (marker == '$') {
			...
			if (lower.contains("timelimit") || lower.contains("limit") || lower.contains("count")
					|| lower.contains("sort")) {
